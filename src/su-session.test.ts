...
import {
  ...
  type SuSessionCapabilitySupport,
...
const identity = <B extends SuSessionBackend>(
  backend: B,
): SuSessionIdentity<B> => ({
...
const commandSamples = {
  ...
  focus: { ...commandBase, type: "focus", commandId: "c5" },
    ...
    type: "end",
    commandId: "c6",
    reason: "owner ended session",
  },
} satisfies {
  [K in SuSessionCommandType]: Extract<SuSessionCommand<"claude">, { type: K }>;
};
...
const eventSamples = {
  ...
  [K in SuSessionEventType]: Extract<SuSessionEvent<"claude">, { type: K }>;
};

type ClaudeEvent = SuSessionEvent<"claude">;
type CommandResultEvent = Extract<ClaudeEvent, { type: "command_result" }>;
type TranscriptEvent = Extract<ClaudeEvent, { type: "transcript" }>;
type ToolEvent = Extract<ClaudeEvent, { type: "tool" }>;
type CardEvent = Extract<ClaudeEvent, { type: "card" }>;

const commandResultSamples = {
  accepted: {
    ...eventBase,
    type: "command_result",
    commandId: "c1",
  ...
  completed: eventSamples.command_result,
  refused: {
    ...eventBase,
    type: "command_result",
    commandId: "c4",
    ...
    refusal: {
      code: "unsupported",
      message: "backend has no native fork",
      retryable: false,
    },
  },
} satisfies {
  [S in CommandResultEvent["status"]]: Extract<
    CommandResultEvent,
    { status: S }
  >;
};

const transcriptPhaseSamples = {
  started: {
    ...eventBase,
    type: "transcript",
    phase: "started",
  ...
  [P in TranscriptEvent["phase"]]: Extract<TranscriptEvent, { phase: P }>;
};

const toolPhaseSamples = {
  started: {
    ...eventBase,
    type: "tool",
    phase: "started",
    ...
    callId: "tool-1",
    name: "coord:orient",
    input: { afterCompaction: true },
  },
  completed: eventSamples.tool,
} satisfies { [P in ToolEvent["phase"]]: Extract<ToolEvent, { phase: P }> };

const cardPhaseSamples = {
  opened: eventSamples.card,
  closed: {
    ...eventBase,
    type: "card",
    phase: "closed",
...
} satisfies { [P in CardEvent["phase"]]: Extract<CardEvent, { phase: P }> };

const capabilityStateSamples = {
  supported,
  conditional: capabilities.features["interactive-cards"],
  unsupported: capabilities.commands.fork,
} satisfies {
  [S in SuSessionCapabilitySupport["state"]]: Extract<
    SuSessionCapabilitySupport,
    { state: S }
  >;
};
...
describe("SU-session wire contract", () => {
  ...
  it("exhaustively round-trips every nested event and capability variant", () => {
    for (const variants of [
      commandResultSamples,
      transcriptPhaseSamples,
      toolPhaseSamples,
      cardPhaseSamples,
      capabilityStateSamples,
    ]) {
